...
const CONVERSION_REASON_LABELS = Object.freeze({
  ...
  persisted_reservation_unavailable:
    "Persisted reservations are not available yet",
  supplier_inquiry_module_unavailable:
    "Supplier inquiries are not available yet",
  recommendation_module_unavailable:
    "Purchase recommendations are not available yet",
});
...
function budgetLabel(
  ...
  const minimum =
    budget.minimumMinor === null
      ? "No minimum"
      : money(budget.minimumMinor, currency);
  const maximum =
    budget.maximumMinor === null
      ? "No maximum"
      : money(budget.maximumMinor, currency);
...
function DemandLoading(): JSX.Element {
  return (
    <div
      aria-label="Loading customer demand workspace"
      className="space-y-4"
      role="status"
    >
...
function CaptureQueryError({
  ...
  return (
    <div
      ...
      role="alert"
    >
      <strong>{label} could not be loaded.</strong> Free-text demand remains
      available, but no catalog or availability value is inferred.
      <button
...
function CaptureDemandDrawer({
  ...
  const [draft, setDraft] = useState<DemandDraft>(() =>
    emptyDraft(initialProductVariantId),
  );
  ...
  // Quick capture shows only the essentials; brand/urgency/follow-up and the
  // rest stay collapsed behind this toggle (kept out of the initial form).
  const [showAdvanced, setShowAdvanced] = useState(false);

  const catalog = useQuery(demandCaptureCatalogQueryOptions(canViewCatalog));
  const initialInPage = catalog.data?.items.some(
    (item) => item.id === initialProductVariantId,
  );
  const initialProduct = useQuery(
    demandCaptureProductQueryOptions(
      initialProductVariantId,
      canViewCatalog &&
        initialProductVariantId.length > 0 &&
        initialInPage !== true,
    ),
  );
  ...
  const selectedProduct = products.find(
    (product) => product.id === draft.productVariantId,
  );
  ...
  // Quick capture additionally requires a phone number. validateDemandDraft keeps
  // phone optional for programmatic callers, so the rule is enforced here in the UI.
  const visibleErrors =
    draft.phone.trim().length === 0 && errors.phone === undefined
  ...
  const submit = (event: FormEvent<HTMLFormElement>): void => {
    ...
    setChecked(true);
    setClientError(null);
    if (hasDemandDraftErrors(visibleErrors)) return;
    try {
      mutation.mutate(
        demandDraftToCreateInput(draft, selectedProduct, adaptedProduct),
      );
    } catch (error) {
      setClientError(
  ...
  return (
    ...
    >
      <button
        aria-label="Close record demand drawer"
      ...
      >
        <header className="flex items-start gap-3 border-b border-line px-5 py-4 sm:px-6">
          ...
          <div>
            <h2 className="font-bold text-ink" id="capture-demand-title">
              Record customer demand
            </h2>
            <p className="mt-0.5 text-xs text-ink-muted">
              Capture the customer’s exact wording; the server rechecks scoped
              stock and price.
            </p>
          </div>
          <button
        ...
        <div className="flex-1 space-y-5 overflow-y-auto px-5 py-5 sm:px-6">
          ...
          {initialProduct.error === null ? null : (
            <CaptureQueryError
              label="Selected catalog product"
              retry={() => void initialProduct.refetch()}
            />
          )}

          <div className="flex flex-wrap items-center justify-between gap-2 rounded-control border border-line bg-surface-subtle/50 px-3 py-2">
            <p className="text-xs text-ink-muted">
              Quick capture — just the requested item and a phone number.
          ...
          <Field
            error={checked ? errors.customerName : undefined}
          ...
          <Field
            error={checked ? errors.requestText : undefined}
            help="This original wording is immutable after capture. Free text is fine — the item need not be in the catalog."
            label="Requested product / item"
          >
            <input
              className={controlClass}
              disabled={mutation.isPending}
              ...
              placeholder="e.g. iPhone 16 Pro 256 (any colour)"
          ...
          {showAdvanced ? (
            <Field
              help="Match a real catalog variant so qualified demand can feed the buying plan."
              label="Product — match from catalog (optional)"
            >
              <select
                className={controlClass}
                disabled={
                  !canViewCatalog || catalog.isPending || mutation.isPending
                }
              ...
              >
                <option value="">
                  {canViewCatalog
                    ...
                    : "Catalog access not granted"}
                </option>
                {catalogSelection.length > 0 &&
                catalogSelection !== "__other" &&
                selectedProduct === undefined ? (
                  <option value={catalogSelection}>
                ...
                {products.map((product) => (
                  <option key={product.id} value={product.id}>
                    ...
                    {product.productModel.name} · {product.name} · {product.sku}
                  </option>
                ))}
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <Field
              error={checked ? visibleErrors.phone : undefined}
              help="Used to reach the customer when stock arrives."
            ...
            <Field
              error={checked ? errors.quantity : undefined}
          ...
          <Field
            error={checked ? errors.budget : undefined}
            help="Exact rupees or a range; 40k–46k is accepted."
            label="Customer budget (optional)"
          >
            <input
              className={controlClass}
              disabled={mutation.isPending}
              ...
              placeholder="e.g. 40000–46000"
              value={draft.budget}
            />
          </Field>

          <Field
            error={checked ? errors.variantDetails : undefined}
            label="Brand / model / variant (optional)"
          >
            <input
              className={controlClass}
              disabled={mutation.isPending}
              maxLength={120}
              onChange={(event) => update("variantDetails", event.target.value)}
              placeholder="Samsung · Galaxy A15 · 256 GB Green"
              value={draft.variantDetails}
            />
          </Field>
          ...
          {showAdvanced ? (
            <>
              <SegmentedControl
                ...
                onChange={(value) => update("ptaPreference", value)}
                options={PTA_OPTIONS}
                value={draft.ptaPreference}
              />
              <SegmentedControl
                label="Urgency"
              ...
              <Field
                error={checked ? errors.followUp : undefined}
          ...
          <Field error={checked ? errors.note : undefined} label="Note">
            <textarea
              ...
              placeholder="Colour preference, timing, price objection…"
              value={draft.note}
            />
          </Field>
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              <label className="flex cursor-pointer items-start gap-2.5 text-sm text-ink-subtle">
                <input
                  ...
                  disabled={mutation.isPending}
                  onChange={(event) =>
                    update("consentToContact", event.target.checked)
                  }
                  type="checkbox"
                />
              ...
              <label className="flex cursor-pointer items-start gap-2.5 text-sm text-ink-subtle">
                <input
                  checked={draft.tradeInInterest}
                  className="mt-0.5 size-4 accent-[var(--accent)]"
                  disabled={mutation.isPending}
                  onChange={(event) =>
                    update("tradeInInterest", event.target.checked)
                  }
                  type="checkbox"
                />
        ...
        <footer className="flex flex-wrap items-center justify-end gap-2 border-t border-line bg-surface-subtle px-5 py-3.5 sm:px-6">
          ...
          >
            ...
            {mutation.isPending ? "Recording…" : "Record demand"}
          </button>
        </footer>
      </form>
    </div>
    ...
    <section className="overflow-hidden rounded-card border border-line bg-surface shadow-card">
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-line px-4 py-4 sm:px-5">
        <div>
          ...
          <p className="mt-0.5 text-xs text-ink-muted">
            Scoped to the current branch and authorized stock locations.
          </p>
        </div>
        <div className="flex flex-wrap items-center gap-2">
          <form className="relative" key={q} onSubmit={submitSearch}>
            ...
            <input
              aria-label="Search demand requests"
              className="min-h-9 w-52 rounded-control border border-line bg-surface pl-9 pr-3 text-xs outline-none focus:border-accent"
              defaultValue={q}
              name="q"
          ...
          <span className="rounded-full bg-surface-subtle px-2.5 py-1 text-xs font-semibold text-ink-muted">
            {pageData === undefined
              ? "—"
              : `${pageData.items.length} of ${pageData.total}`}{" "}
            requests
          </span>
        </div>
      </div>
      <div className="overflow-x-auto border-b border-line bg-surface-subtle px-3 py-2.5">
        <div
          aria-label="Demand outcome filters"
        ...
        >
          {DEMAND_FILTERS.map((item) => (
            ...
            >
              {item.label}{" "}
              <span className="opacity-70">{filterCount(item.id) ?? "—"}</span>
            </button>
          ))}
        </div>
      </div>
              ...
              <tr>
                <td className="px-5 py-12 text-center" colSpan={8}>
                  ...
                  <h3 className="mt-2 font-bold text-ink">
                    Demand requests could not be loaded
                  </h3>
                  <p className="mt-1 text-sm text-ink-muted">
                    {toApiError(list.error).message}
                  </p>
                  <button
                    className="mt-3 inline-flex min-h-9 items-center gap-2 rounded-control border border-line px-3 text-xs font-semibold"
                    onClick={onRetry}
                    type="button"
                  >
              ...
              pageData?.items.map((request) => (
                <tr className="hover:bg-surface-subtle/70" key={request.id}>
                  ...
                  <td className="whitespace-nowrap px-4 py-3 text-xs text-ink-muted">
                    {dateTimeLabel(request.requestedAt, timezone)}
                  </td>
                  <td className="px-4 py-3 text-sm font-semibold text-ink-subtle">
                    {request.contact.customerName ??
                      request.contact.customerPhone ??
                      "Walk-in / private"}
                  </td>
                  <td className="max-w-sm px-4 py-3">
                    ...
                    >
                      <span className="block text-sm font-semibold text-ink">
                        {request.item.match === "matched"
                          ? request.item.productVariant.displayName
                          : request.item.rawRequestText}
                      </span>
                      {request.item.match === "matched" ? (
                        <span className="mt-0.5 block max-w-sm truncate text-xs text-ink-muted">
                          “{request.item.rawRequestText}”
                        </span>
                      ) : null}
                    </button>
                  ...
                  <td className="whitespace-nowrap px-4 py-3 text-xs text-ink-muted">
                    {dateLabel(request.followUpOn, timezone)}
                  </td>
                </tr>
              ))
            )}
      ...
      {pageData !== undefined && pageData.totalPages > 1 ? (
        <div className="flex items-center justify-between border-t border-line px-4 py-3 text-xs text-ink-muted">
          ...
          <div className="flex gap-2">
            <button
              className="min-h-8 rounded-control border border-line px-3 font-semibold disabled:opacity-40"
              disabled={page <= 1}
  ...
  return (
    <section className="rounded-card border border-line bg-surface px-5 py-12 text-center shadow-card">
      ...
      <h2 className="mt-3 font-bold text-ink">
        Demand request history is restricted
      </h2>
      <p className="mx-auto mt-1 max-w-lg text-sm leading-6 text-ink-muted">
        This role can capture a customer request but cannot read the existing
        branch ledger.
      </p>
      {canCreate ? (
        <button
          className="mt-4 inline-flex min-h-9 items-center gap-2 rounded-control bg-accent px-3.5 text-xs font-semibold text-white"
          onClick={onCapture}
          type="button"
        >
          <PlusIcon className="size-4" /> Record demand
...
function DetailRow({
  ...
  return (
    <div className="flex items-start justify-between gap-4 border-b border-line px-4 py-3 text-sm last:border-b-0">
      <span className="text-ink-muted">{label}</span>
      <span className="max-w-[65%] text-right font-semibold text-ink-subtle">
        {value}
      </span>
    </div>
  );
...
function DemandDetailDrawer({
  ...
  const transition = useMutation({
    mutationFn: (request: DemandRequestDetail) =>
      transitionDemandRequestStatus(id, {
        status: z.enum(DEMAND_MANUAL_STATUS_TARGETS).parse(targetStatus),
        outcome,
        lostSaleReason: LOST_OUTCOMES.has(outcome)
  ...
  const actionPending =
    transition.isPending || followUp.isPending || conversion.isPending;
  ...
  return (
    ...
    >
      <button
        aria-label="Close demand detail drawer"
        className="absolute inset-0"
        onClick={onClose}
        type="button"
      />
      ...
      >
        <header className="flex items-start gap-3 border-b border-line px-5 py-4 sm:px-6">
          ...
          <div>
            <h2 className="font-bold text-ink" id="demand-detail-title">
              {request?.requestNumber ?? "Demand request"}
            </h2>
            <p className="mt-0.5 text-xs text-ink-muted">
              Original request, capture evidence, follow-ups and conversion
            </p>
          </div>
          <button
            aria-label="Close drawer"
        ...
        <div className="flex-1 space-y-5 overflow-y-auto px-5 py-5 sm:px-6">
          ...
          {detail.isError ? (
            <div
              ...
              role="alert"
            >
              <p className="font-bold">Demand request could not be loaded</p>
              <p className="mt-1">{toApiError(detail.error).message}</p>
              <button
          ...
          {actionError === null ? null : (
            <div
              ...
              role="alert"
            >
              <p className="font-bold">Demand action could not be completed</p>
              <p className="mt-1">{toApiError(actionError).message}</p>
            </div>
          )}
          {request === undefined ? null : (
            <>
              <section className="overflow-hidden rounded-card border border-line">
                <DetailRow
                  label="Requested"
                  value={dateTimeLabel(request.requestedAt, timezone)}
                />
                <DetailRow
                  label="Customer"
                  value={
                ...
                <DetailRow
                  ...
                  value={`“${request.item.rawRequestText}”`}
                />
                <DetailRow
                  label="Catalog match"
                  value={
                    request.item.match === "matched"
                      ? `${request.item.productVariant.displayName} · ${request.item.productVariant.sku}`
                      : "Not in catalog"
                  }
                />
                <DetailRow
                  label="Quantity / budget"
                  value={`${request.quantity} · ${budgetLabel(request.budget, currency)}`}
                />
                <DetailRow
                  label="Urgency / channel"
                  value={`${title(request.urgency)} · ${title(request.channel)}`}
                />
                <DetailRow label="Status" value={title(request.status)} />
                <DetailRow
                  label="Outcome"
                  value={<OutcomeBadge outcome={request.outcome} />}
                />
                <DetailRow
                  label="Capture availability"
                  value={title(request.availabilitySnapshot.state)}
                />
                <DetailRow
                  label="Buying plan"
                  value={
                    request.qualifiedForBuyingPlan
                      ? request.countsTowardForecast
                        ? "Qualified · counted"
                ...
                <DetailRow
                  label="PTA preference"
                  value={title(request.ptaPreference)}
                />
                <DetailRow
                  label="Trade-in interest"
                  value={request.tradeInInterest ? "Yes" : "No"}
                />
                <DetailRow
                  label="Follow-up due"
                  value={dateLabel(request.followUpOn, timezone)}
                />
                <DetailRow label="Note" value={request.note ?? "—"} />
              </section>

              {request.conversion === null ? null : (
                <div className="rounded-control border border-positive/25 bg-positive-soft p-3 text-sm text-positive">
                  <strong>Converted to sale.</strong> Linked sale:{" "}
                  <span className="font-mono">
                    {request.conversion.targetId}
                  </span>
                </div>
              )}
              ...
              <section>
                ...
                {request.followUps.length === 0 ? (
                  <p className="mt-2 rounded-control border border-dashed border-line p-4 text-center text-xs text-ink-muted">
                    {canViewCustomers
                      ? "No follow-ups recorded yet."
                      : "Customer contact history is private for this role."}
                  </p>
                ) : (
                  <div className="mt-2 space-y-2">
                    {request.followUps.map((item) => (
                      <article
                        className="rounded-control border border-line p-3 text-xs"
                        key={item.id}
                      >
                        <div className="flex items-center justify-between gap-3">
                          <strong className="text-ink">
                            {title(item.result)} · {title(item.channel)}
                          </strong>
                          <span className="text-ink-muted">
                            {dateTimeLabel(item.occurredAt, timezone)}
                          </span>
                        </div>
                        <p className="mt-1 leading-5 text-ink-subtle">
                          {item.note}
                        </p>
                        <p className="mt-1 text-ink-muted">
                          By {item.createdBy.displayName}
                          {item.nextFollowUpOn === null
                            ? ""
                            : ` · Next ${dateLabel(item.nextFollowUpOn, timezone)}`}
                        </p>
                      </article>
                    ))}
                  </div>
              ...
              {!canManage ? null : (
                <section className="space-y-3 rounded-card border border-line p-4">
                  <div>
                    ...
                    <p className="mt-0.5 text-xs text-ink-muted">
                      Every status change is version-checked and audited.
                    </p>
                  </div>
                  {STATUS_TRANSITIONS[request.status].length === 0 ? (
                    <p className="rounded-control bg-surface-subtle p-3 text-xs text-ink-muted">
                      This request is in a terminal status.
                    </p>
                  ) : (
                    <>
                      <div className="grid gap-3 sm:grid-cols-2">
                        ...
                        <Field label="Outcome">
                          <select
                            ...
                            disabled={actionPending}
                            onChange={(event) =>
                              setOutcome(event.target.value as DemandOutcome)
                            }
                            value={outcome}
                          >
                            {DEMAND_OUTCOMES.filter(
                              (value) => value !== "sold_immediately",
                            ).map((value) => (
                      ...
                      {LOST_OUTCOMES.has(outcome) ? (
                        <Field label="Why the sale was lost">
                          <textarea
                      ...
                      <button
                        className="min-h-9 rounded-control bg-accent px-3.5 text-xs font-semibold text-white disabled:opacity-40"
                        disabled={
                          actionPending ||
                          targetStatus === "" ||
                          (LOST_OUTCOMES.has(outcome) &&
                        ...
                        onClick={() => transition.mutate(request)}
                        type="button"
                      >
              ...
              {!canManage || !canViewCustomers ? null : (
                <section className="space-y-3 rounded-card border border-line p-4">
                  <div>
                    ...
                    <p className="mt-0.5 text-xs text-ink-muted">
                      Adds a new history entry; earlier contact evidence is
                      never overwritten.
                    </p>
                  </div>
                  <div className="grid gap-3 sm:grid-cols-2">
                    <Field label="Channel">
                      <select
                        ...
                        onChange={(event) =>
                          setFollowUpChannel(
                            event.target.value as DemandChannel,
                          )
                        }
                        value={followUpChannel}
                      >
                        {DEMAND_CHANNELS.map((value) => (
                          <option key={value} value={value}>
                            {title(value)}
                          </option>
                        ))}
                    ...
                    <Field label="Result">
                      <select
                        ...
                        onChange={(event) =>
                          setFollowUpResult(
                            event.target.value as DemandFollowUpResult,
                          )
                        }
                        value={followUpResult}
                      >
                        {DEMAND_FOLLOW_UP_RESULTS.map((value) => (
                          <option key={value} value={value}>
                            {title(value)}
                          </option>
                        ))}
                  ...
                  <button
                    className="min-h-9 rounded-control bg-accent px-3.5 text-xs font-semibold text-white disabled:opacity-40"
                    disabled={actionPending || followUpNote.trim().length === 0}
                    onClick={() => followUp.mutate()}
                    type="button"
                  >
                    {followUp.isPending ? "Recording…" : "Add follow-up"}
                  </button>
                </section>
              )}
              ...
              {!canManage || request.status === "converted_to_sale" ? null : (
                <section className="space-y-3 rounded-card border border-line p-4">
                  <div>
                    ...
                    <p className="mt-0.5 text-xs text-ink-muted">
                      Only an already-posted sale can be linked atomically
                      today.
                    </p>
                  </div>
                  <div className="flex flex-wrap gap-2">
                    {capabilities.data
                      ...
                      .map((capability) => (
                        <button
                          className="rounded-control border border-line bg-surface-subtle px-2.5 py-1.5 text-[0.6875rem] font-semibold text-ink-muted"
                          disabled
                          key={capability.target}
                          title={CONVERSION_REASON_LABELS[capability.reason]}
                          type="button"
                        >
                          {title(capability.target)} · unavailable
                        </button>
                      ))}
                  </div>
                  {capabilities.isError ? (
                    <p className="text-xs text-warning">
                      Conversion capabilities could not be verified; no
                      unsupported action is enabled.
                    </p>
                  ) : null}
                  <div className="flex flex-wrap gap-2">
                    <Link
                      className="inline-flex min-h-9 items-center rounded-control border border-line px-3 text-xs font-semibold text-accent no-underline"
                      href={
                        request.item.match === "matched"
                          ? `/sell?productVariantId=${encodeURIComponent(request.item.productVariant.id)}`
                          : "/sell"
                      }
                    >
                      Open Sell →
                    ...
                    <input
                      aria-label="Posted sale ID"
                      className="min-h-9 min-w-60 flex-1 rounded-control border border-line px-3 font-mono text-xs outline-none focus:border-accent"
                      onChange={(event) => setSaleId(event.target.value.trim())}
                      placeholder="Paste posted sale UUID"
                      value={saleId}
                    />
                    <button
                      className="min-h-9 rounded-control bg-accent px-3.5 text-xs font-semibold text-white disabled:opacity-40"
                      disabled={actionPending || !saleIdValid}
                      onClick={() => conversion.mutate(request)}
                      type="button"
                    >
                      {conversion.isPending ? "Linking…" : "Link sale"}
                    </button>
                  </div>
                  {saleId.length > 0 && !saleIdValid ? (
                    <p className="text-xs text-negative">
                      Enter a valid posted sale UUID.
                    </p>
                  ) : null}
                </section>
              )}
...
function DemandAuthenticatedWorkspace({
  auth,
}: {
  ...
  const initialProductVariantId = z.uuid().safeParse(productParameter).success
    ? productParameter
    : "";
  ...
  const baseParameters = {
    page,
    pageSize: 25,
    view: demandViewForFilter(filter),
    sort: "requested_at" as const,
    direction: "desc" as const,
    ...(q.length === 0 ? {} : { q }),
  };
  const ledger = useQuery(
    demandRequestsQueryOptions(baseParameters, capabilities.canView),
  );
  const countQueries = useQueries({
    queries: DEMAND_FILTERS.map((item) =>
      demandRequestsQueryOptions(
        {
          ...baseParameters,
          page: 1,
          pageSize: 1,
          view: demandViewForFilter(item.id),
        },
  ...
  const kpis =
    ledger.data?.kpis ??
    countQueries.find((query) => query.data !== undefined)?.data?.kpis;

  if (!capabilities.canView && !capabilities.canCreate) {
    return (
      <CatalogForbiddenState
        description="This workspace requires demand.view or demand.create. No demand or capture-dependency request was sent."
  ...
  const filterCount = (target: DemandFilter): number | undefined => {
    const index = DEMAND_FILTERS.findIndex(
  ...
  const saved = (record: DemandRequestDetail): void => {
    ...
    setSavedNotice(
      `${record.requestNumber} was recorded and included in live Demand analytics.`,
    );
    ...
    <div className="space-y-4">
      <header className="rounded-card border border-line bg-surface p-5 shadow-card sm:p-6">
        <div className="flex flex-wrap items-start justify-between gap-4">
          ...
          <div className="flex flex-wrap items-center gap-2">
            <Link
              className="inline-flex min-h-10 items-center rounded-control border border-line bg-surface px-3.5 text-sm font-semibold text-ink-subtle no-underline hover:bg-surface-subtle"
              href="/intelligence"
            >
              View buying plan →
            </Link>
      ...
      {savedNotice === null ? null : (
        <div
          className="flex items-center gap-2 rounded-control border border-positive/25 bg-positive-soft px-4 py-3 text-sm text-positive"
          role="status"
        >
      ...
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        <KpiCard
          ...
          meta={
            capabilities.canView
              ? `As of ${kpis?.businessDate ?? "today"}`
              : "Restricted"
          }
          onClick={capabilities.canView ? () => setFilter("all") : undefined}
          value={capabilities.canView ? (kpis?.totalRequests ?? "—") : "—"}
        />
        <KpiCard
          emphasis="negative"
          label="Unavailable · missed"
          meta={capabilities.canView ? "Qualified missed demand" : "Restricted"}
          onClick={
            capabilities.canView ? () => setFilter("unavailable") : undefined
          }
          value={capabilities.canView ? (kpis?.unavailableMissed ?? "—") : "—"}
        />
        <KpiCard
          emphasis="positive"
          label="Reserved / quotation"
          meta={capabilities.canView ? "Conversion signals" : "Restricted"}
          onClick={
            capabilities.canView ? () => setFilter("reserved") : undefined
          }
          value={capabilities.canView ? (kpis?.reservedOrQuoted ?? "—") : "—"}
        />
        <KpiCard
          href="/tasks"
          label="Follow-ups due"
          meta="Open Tasks →"
          value={capabilities.canView ? (kpis?.followUpsDue ?? "—") : "—"}
        />
      </div>
      ...
      <div className="flex items-start gap-3 rounded-card border border-warning/30 bg-warning-soft px-4 py-3.5 text-sm text-warning">
        ...
        <div>
          <p className="font-bold">
            Unavailable requests feed the reorder engine
          </p>
          <p className="mt-0.5 leading-5">
            The server qualifies and deduplicates captured misses before they
            count toward the buying plan.{" "}
            <Link className="font-bold" href="/intelligence">
              Open the buying plan →
            </Link>
          </p>
        </div>
        <span className="ml-auto hidden shrink-0 items-center gap-1.5 rounded-full bg-surface px-2.5 py-1 text-xs font-bold text-warning sm:inline-flex">
          <ShieldCheckIcon className="size-4" /> Live · scoped · audited
        </span>
      </div>
      ...
      {capabilities.canView ? (
        <RequestsLedger
          ...
          onPage={(nextPage) => replaceParameters({ page: String(nextPage) })}
          onRetry={() => void ledger.refetch()}
          onSearch={(value) =>
            replaceParameters({ q: value.trim() || null, page: null })
          }
          page={page}
          q={q}
      ...
      {captureOpen && capabilities.canCreate ? (
        <CaptureDemandDrawer
          canViewCatalog={capabilities.canViewCatalog}
          canViewInventory={capabilities.canViewInventory}
          canViewPricing={capabilities.canViewPricing}
          initialProductVariantId={initialProductVariantId}
          onClose={() => setCaptureOpen(false)}
          onSaved={saved}
        />
      ) : null}
      {selectedRequestId.length > 0 && capabilities.canView ? (
        <DemandDetailDrawer
...
export function DemandWorkspace(): JSX.Element {
  ...
  if (auth.isError || auth.data === undefined) {
    return (
      <CatalogForbiddenState
        description="The current session could not be checked, so no demand, catalog, pricing, or customer data was requested. Restore the API connection and retry."
