...
describe("money — minor unit conversion", () => {
  ...
  it("round-trips through the major string form", () => {
    for (const value of ["0.00", "1.00", "1234.56", "-45.50", "999999.99"]) {
      expect(toMajorString(fromMajor(value))).toBe(
...
describe("money — arithmetic", () => {
  ...
  it("sums a list", () => {
    expect(sum([fromMajor("1.10"), fromMajor("2.20"), fromMajor("3.30")])).toBe(
      660,
    );
    expect(sum([])).toBe(0);
  });
  ...
  it("rejects fractional quantities", () => {
    expect(() => multiplyByQuantity(fromMajor("10.00"), 1.5)).toThrow(
      MoneyError,
    );
  });
});
...
describe("money — allocation", () => {
  ...
  it("rejects invalid weights", () => {
    ...
    expect(() => allocateByWeights(fromMajor("10"), [0, 0])).toThrow(
      MoneyError,
    );
    expect(() => allocateByWeights(fromMajor("10"), [-1, 2])).toThrow(
      MoneyError,
    );
  });
...
describe("money — formatting", () => {
  ...
  it("formats negatives and suppresses the symbol on request", () => {
    ...
    expect(formatMoney(fromMajor("10.00"), "PKR", { withSymbol: false })).toBe(
      "10.00",
    );
  });
