...
describe('Health endpoints (HTTP)', () => {
  ...
  describe('GET /api/v1/health', () => {
    ...
    it('leaks no configuration or secrets', async () => {
      ...
      for (const forbidden of ['password', 'secret', 'postgresql://', 'session_secret', 'database_url']) {
        expect(body, `health response must not contain "${forbidden}"`).not.toContain(forbidden);
      }
    });
  });

  describe('GET /api/v1/health/ready', () => {
    it('returns 200 when no dependency is registered yet', async () => {
      const response = await request(app.getHttpServer()).get('/api/v1/health/ready').expect(200);
      expect(response.body).toMatchObject({ status: 'ok', dependencies: {} });
    });
  });
