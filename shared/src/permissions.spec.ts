...
describe("role decisions — owner", () => {
  it("has full access", () => {
    expect(permissionsForRole(ROLES.OWNER)).toHaveLength(
  ...
  it("is the only role that can manage roles or reopen a reviewed cash session", () => {
    for (const role of Object.values(ROLES)) {
      ...
      expect(
        hasPermission(permissionsForRole(role), PERMISSIONS.ROLES_MANAGE),
      ).toBe(false);
      expect(
        hasPermission(
          permissionsForRole(role),
          PERMISSIONS.CASH_SESSIONS_REOPEN,
        ),
      ).toBe(false);
    }
  });
  ...
  it("is the only role that can decide on recommendations (13_ §18: no auto-approval)", () => {
    for (const role of Object.values(ROLES)) {
      ...
      expect(
        hasPermission(
          permissionsForRole(role),
          PERMISSIONS.RECOMMENDATIONS_DECIDE,
        ),
      ).toBe(false);
    }
  });
...
describe("role decisions — salesperson", () => {
  ...
  it("can sell and capture demand", () => {
    ...
    expect(hasPermission(salesperson, PERMISSIONS.INVENTORY_RESERVE)).toBe(
      true,
    );
  });

  it("cannot see supplier cost or profit (13_ §8)", () => {
    expect(hasPermission(salesperson, PERMISSIONS.CATALOG_VIEW_COST)).toBe(
      false,
    );
    expect(hasPermission(salesperson, PERMISSIONS.INVENTORY_VIEW_COST)).toBe(
      false,
    );
    expect(hasPermission(salesperson, PERMISSIONS.SALES_VIEW_PROFIT)).toBe(
      false,
    );
    expect(hasPermission(salesperson, PERMISSIONS.REPORTS_VIEW_FINANCIAL)).toBe(
      false,
    );
  });
  ...
  it("cannot override discounts or grant credit without authorization", () => {
    expect(
      hasPermission(salesperson, PERMISSIONS.SALES_DISCOUNT_OVERRIDE),
    ).toBe(false);
    expect(hasPermission(salesperson, PERMISSIONS.SALES_CREDIT)).toBe(false);
  });

  it("cannot adjust stock", () => {
    expect(hasPermission(salesperson, PERMISSIONS.INVENTORY_ADJUST)).toBe(
      false,
    );
  });
});
...
describe("role decisions — cashier", () => {
  ...
  it("can collect payments, run a drawer and record external services", () => {
    ...
    expect(hasPermission(cashier, PERMISSIONS.EXTERNAL_SERVICES_RECORD)).toBe(
      true,
    );
  ...
  it("cannot review its own cash session (segregation of duties, 13_ §14)", () => {
    expect(hasPermission(cashier, PERMISSIONS.CASH_SESSIONS_REVIEW)).toBe(
      false,
    );
  });

  it("cannot change fee rules it operates under", () => {
    expect(hasPermission(cashier, PERMISSIONS.EXTERNAL_FEE_RULES_MANAGE)).toBe(
      false,
    );
  });
});
...
describe("role decisions — accountant", () => {
  ...
  it("can read financial reports and the ledger", () => {
    expect(hasPermission(accountant, PERMISSIONS.REPORTS_VIEW_FINANCIAL)).toBe(
      true,
    );
  ...
  it("has no operational posting rights (13_ §8)", () => {
    ...
    expect(hasPermission(accountant, PERMISSIONS.PURCHASES_RECEIVE)).toBe(
      false,
    );
...
describe("role decisions — technician", () => {
  it("cannot view unrelated financial data (01_PRD §4)", () => {
    ...
    expect(hasPermission(technician, PERMISSIONS.REPORTS_VIEW_FINANCIAL)).toBe(
      false,
    );
    expect(hasPermission(technician, PERMISSIONS.SALES_VIEW_PROFIT)).toBe(
      false,
    );
    expect(hasPermission(technician, PERMISSIONS.INVENTORY_VIEW_COST)).toBe(
      false,
    );
  });
});
...
describe("hasPermission", () => {
  it("accepts arrays and sets", () => {
    expect(
      hasPermission([PERMISSIONS.SALES_POST], PERMISSIONS.SALES_POST),
    ).toBe(true);
    expect(
      hasPermission(new Set([PERMISSIONS.SALES_POST]), PERMISSIONS.SALES_POST),
    ).toBe(true);
    expect(
      hasPermission(
        new Set([PERMISSIONS.SALES_POST]),
