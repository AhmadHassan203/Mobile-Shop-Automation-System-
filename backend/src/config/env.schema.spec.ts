...
describe('environment validation', () => {
  ...
  it('coerces numeric strings, since process.env values are always strings', () => {
    const env = validateEnv({ ...BASE, API_PORT: '5000', SESSION_TTL_HOURS: '8' });
  ...
  it('coerces boolean-ish strings', () => {
    ...
    expect(validateEnv({ ...BASE, LOG_PRETTY: 'false' }).LOG_PRETTY).toBe(false);
    expect(validateEnv({ ...BASE, LOG_PRETTY: '0' }).LOG_PRETTY).toBe(false);
  });

  it('rejects a missing database URL', () => {
    expect(() => validateEnv({ SESSION_SECRET: 'a'.repeat(64) })).toThrow(/DATABASE_URL/);
  });
  ...
  it('rejects a short session secret', () => {
    expect(() => validateEnv({ ...BASE, SESSION_SECRET: 'too-short' })).toThrow(/at least 32/);
  });
  ...
  it('reports every problem at once rather than one per restart', () => {
    let message = '';
    try {
      validateEnv({ DATABASE_URL: 'nope', SESSION_SECRET: 'short', API_PORT: '99999' });
    } catch (error) {
...
describe('production guardrails', () => {
  const PROD = { ...BASE, NODE_ENV: 'production', SESSION_COOKIE_SECURE: 'true' };
  ...
  it('refuses to boot production with an insecure session cookie', () => {
    ...
    expect(() => validateEnv({ ...PROD, SESSION_COOKIE_SECURE: 'false' })).toThrow(
      /SESSION_COOKIE_SECURE must be true in production/,
    );
  });

  it('refuses to boot production with the placeholder session secret', () => {
    expect(() => validateEnv({ ...PROD, SESSION_SECRET: 'CHANGE_ME_32_BYTE_HEX_SECRET_PLACEHOLDER' })).toThrow(
      /placeholder/,
    );
  });
  ...
  it('refuses to boot production with the placeholder database URL', () => {
    expect(() =>
      validateEnv({ ...PROD, DATABASE_URL: 'postgresql://app:CHANGE_ME@localhost:5432/db' }),
    ).toThrow(/placeholder/);
  });

  it('refuses to boot production with pretty (unstructured) logs', () => {
    expect(() => validateEnv({ ...PROD, LOG_PRETTY: 'true' })).toThrow(/structured JSON logs/);
  });

  it('refuses S3 storage without complete credentials', () => {
    expect(() => validateEnv({ ...PROD, STORAGE_DRIVER: 's3' })).toThrow(/S3_BUCKET/);
    expect(() =>
      validateEnv({
        ...PROD,
        STORAGE_DRIVER: 's3',
  ...
  it('allows the same relaxed settings outside production', () => {
    ...
    expect(() => validateEnv({ ...BASE, NODE_ENV: 'development', LOG_PRETTY: 'true' })).not.toThrow();
    expect(() => validateEnv({ ...BASE, NODE_ENV: 'development', SESSION_COOKIE_SECURE: 'false' })).not.toThrow();
  });
});
