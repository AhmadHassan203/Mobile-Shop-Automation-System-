...
describe('business day UTC boundaries', () => {
  it('starts a business day at 19:00 UTC the previous day', () => {
    expect(businessDayStartUtc(parseBusinessDate('2026-07-15')).toISOString()).toBe(
      '2026-07-14T19:00:00.000Z',
    );
  });

  it('ends a business day at the next day start (exclusive)', () => {
    expect(businessDayEndUtc(parseBusinessDate('2026-07-15')).toISOString()).toBe(
...
describe('business date arithmetic', () => {
  it('adds and subtracts days', () => {
    expect(addBusinessDays(parseBusinessDate('2026-07-15'), 1)).toBe('2026-07-16');
    expect(addBusinessDays(parseBusinessDate('2026-07-15'), -1)).toBe('2026-07-14');
    expect(addBusinessDays(parseBusinessDate('2026-07-15'), 0)).toBe('2026-07-15');
  });

  it('crosses month and year boundaries', () => {
    expect(addBusinessDays(parseBusinessDate('2026-07-31'), 1)).toBe('2026-08-01');
    expect(addBusinessDays(parseBusinessDate('2026-12-31'), 1)).toBe('2027-01-01');
    expect(addBusinessDays(parseBusinessDate('2028-02-28'), 1)).toBe('2028-02-29'); // leap year
  });

  it('rejects fractional day counts', () => {
    expect(() => addBusinessDays(parseBusinessDate('2026-07-15'), 1.5)).toThrow(DateTimeError);
  });

  it('counts days between dates, signed', () => {
    expect(businessDaysBetween(parseBusinessDate('2026-07-01'), parseBusinessDate('2026-07-31'))).toBe(30);
    expect(businessDaysBetween(parseBusinessDate('2026-07-31'), parseBusinessDate('2026-07-01'))).toBe(-30);
    expect(businessDaysBetween(parseBusinessDate('2026-07-15'), parseBusinessDate('2026-07-15'))).toBe(0);
  });
});
...
describe('rolling windows', () => {
  ...
  it('rejects a non-positive window', () => {
    expect(() => rollingWindow(parseBusinessDate('2026-07-15'), 0)).toThrow(DateTimeError);
    expect(() => rollingWindow(parseBusinessDate('2026-07-15'), -5)).toThrow(DateTimeError);
  });
});
...
describe('stock aging', () => {
  ...
  it('is zero on the same business day', () => {
    expect(ageInDays(new Date('2026-07-15T05:00:00Z'), new Date('2026-07-15T14:00:00Z'))).toBe(0);
  });
});
