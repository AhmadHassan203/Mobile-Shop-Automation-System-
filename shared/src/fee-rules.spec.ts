...
describe("fee calculation — approved default business rules", () => {
  ...
  it("scales linearly across whole blocks", () => {
    expect(
      toMajorString(calculateFee(fromMajor("5000"), SEND_RULE).feeMinor),
    ).toBe("50.00");
    expect(
      toMajorString(calculateFee(fromMajor("5000"), WITHDRAWAL_RULE).feeMinor),
    ).toBe("100.00");
    expect(
      toMajorString(calculateFee(fromMajor("25000"), SEND_RULE).feeMinor),
    ).toBe("250.00");
  });
...
describe("fee calculation — partial block behavior (ASM-003)", () => {
  ...
  it("proportional_block pro-rates the partial thousand instead", () => {
    ...
    expect(toMajorString(calculateFee(fromMajor("1500"), rule).feeMinor)).toBe(
      "15.00",
    );
    expect(toMajorString(calculateFee(fromMajor("500"), rule).feeMinor)).toBe(
      "5.00",
    );
  });
...
describe("fee calculation — parity with the approved prototype", () => {
  ...
  const PROTOTYPE_SEND: FeeRule = {
    ...SEND_RULE,
    minFeeMinor: fromMajor("10"),
  };
  const PROTOTYPE_RECEIVED: FeeRule = {
    ...WITHDRAWAL_RULE,
    minFeeMinor: fromMajor("20"),
  };

  function prototypeSlabFee(
    amountRupees: number,
    blockSize: number,
    feePerBlock: number,
    minimumFee: number,
    ...
    return Math.round(
      Math.max(Math.ceil(amountRupees / blockSize) * feePerBlock, minimumFee),
    );
  }

  it("reproduces the prototype SLAB fee for send across a range of amounts", () => {
    for (const rupees of [
      1, 500, 999, 1000, 1001, 1500, 2000, 4999, 5000, 25000, 100000,
    ]) {
      const expected = prototypeSlabFee(rupees, 1000, 10, 10);
      const actual = calculateFee(
        fromMajor(String(rupees)),
        PROTOTYPE_SEND,
      ).feeMinor;
  ...
  it("reproduces the prototype SLAB fee for withdrawal across a range of amounts", () => {
    for (const rupees of [1, 500, 1000, 1500, 3000, 7500, 50000]) {
      ...
      expect(toMajorString(actual), `withdrawal ${rupees}`).toBe(
        expected.toFixed(2),
      );
    }
  });
...
describe("fee calculation — other modes", () => {
  it("applies a flat fee regardless of principal", () => {
    ...
    expect(toMajorString(calculateFee(fromMajor("1000"), rule).feeMinor)).toBe(
      "50.00",
    );
    expect(
      toMajorString(calculateFee(fromMajor("100000"), rule).feeMinor),
    ).toBe("50.00");
  });
...
describe("fee calculation — min/max clamping", () => {
  ...
  it("explains why a clamp applied", () => {
    const rule: FeeRule = { ...SEND_RULE, minFeeMinor: fromMajor("25") };
    expect(calculateFee(fromMajor("1000"), rule).explanation).toContain(
      "minimum fee",
    );
  });
});
...
describe("fee calculation — invalid configuration", () => {
  ...
  it("rejects block modes without a positive block size", () => {
    expect(() =>
      calculateFee(fromMajor("1000"), {
        mode: "per_started_block",
        feePerBlockMinor: toMinor(10),
      }),
  ...
  it("rejects percentage mode without a rate", () => {
    expect(() =>
      calculateFee(fromMajor("1000"), { mode: "percentage" }),
    ).toThrow(FeeRuleError);
  });
});
...
describe("service profit", () => {
  ...
  it("treats omitted costs as zero", () => {
    expect(
      toMajorString(
        calculateServiceProfit({ customerFeeMinor: fromMajor("20") }),
      ),
    ).toBe("20.00");
  });
