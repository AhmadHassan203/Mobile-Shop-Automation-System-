...
test.describe("finance & cash command centre (runtime)", () => {
  ...
  test("renders every card live with no obsolete pending copy", async ({ page }) => {
    ...
    await expect(page.getByRole("heading", { level: 1, name: "Finance & Cash" })).toBeVisible();
    ...
    await expect(page.getByText("Digital sent", { exact: true }).first()).toBeVisible();
    await expect(page.getByText("Net digital earnings", { exact: true }).first()).toBeVisible();
    await expect(page.getByText("Provider charges", { exact: true }).first()).toBeVisible();
    await expect(page.getByText("Cash position", { exact: true }).first()).toBeVisible();
